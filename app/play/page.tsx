...
import RedoxChessEngine from "@/src/utils/redoxchessEngine";
...
const SYSTEM_PROMPT = `You are Khadeer Shaik, a passionate AI & Full-Stack Developer from India. You are NOT an AI assistant - you ARE Khadeer himself chatting with visitors on your portfolio website.`;
...
export default function PlayPage() {
  ...
  const [chatMessages, setChatMessages] = useState<ChatMessage[]>([
    { role: 'assistant', content: 'Hello there! I am Khadeer Shaik 👋 Ask me anything you want to know!' }
  ]);
  ...
  const renderPiece = (piece: { type: PieceSymbol; color: Color } | null) => {
    ...
    if (!svg) return null;
    return <div className="w-full h-full p-1" dangerouslySetInnerHTML={{ __html: svg }} />;
  };
  ...
  const renderCapturedPieces = (pieces: string[], color: Color) => {
    return pieces.map((piece, index) => {
      ...
      const svg = PIECES[key];
      return (
        <div key={index} className="w-5 h-5 sm:w-6 sm:h-6 inline-block" dangerouslySetInnerHTML={{ __html: svg || '' }} />
      );
    });
  };
  ...
  return (
    <div className="play-page min-h-screen bg-[#0b080c] text-white flex flex-col justify-between pt-24">
      <Navbar />

      <div className="p-4 sm:p-8 flex-1 flex flex-col items-center pt-6">

        <div className="chess-container w-full max-w-7xl grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
          {/* Chat Panel - Left Side */}
          <div className="chat-panel lg:col-span-4 rounded-3xl bg-white/5 border border-white/10 p-4 flex flex-col h-[500px] lg:h-[600px]">
            <div className="chat-header pb-3 mb-3 border-b border-white/10 font-bold text-accent font-geist flex items-center gap-2">
              <span>💬 Talk with me</span>
            </div>
            <div className="chat-messages flex-1 overflow-y-auto space-y-3 pr-2">
              {chatMessages.map((msg, index) => (
                <div key={index} className={`chat-message flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                  <div className={`message-content p-3 rounded-2xl text-sm max-w-[85%] ${msg.role === 'user' ? 'bg-accent text-black font-medium' : 'bg-white/10 text-slate-200'}`}>
                    {msg.content}
                  </div>
                </div>
              ))}
              {isTyping && (
                <div className="chat-message flex justify-start">
                  <div className="message-content p-3 rounded-2xl bg-white/10 text-slate-400 text-xs italic">
                    Khadeer is typing...
                  </div>
                </div>
              )}
            </div>
            <div className="chat-input-area pt-3 border-t border-white/10 flex gap-2">
              <input
                type="text"
                className="chat-input flex-1 px-4 py-2 rounded-full bg-white/10 border border-white/20 text-sm text-white placeholder-slate-400 focus:outline-none focus:border-accent"
                ...
                data-cursor="disable"
              />
              <button className="chat-send-btn px-4 py-2 rounded-full bg-accent text-black font-bold hover:bg-accent/80 transition-colors" onClick={sendMessage} data-cursor="disable">
                ➤
              </button>
            </div>
          </div>
          ...
          <div className="chess-board-section lg:col-span-5 flex flex-col items-center gap-3">
            {/* Opponent Info */}
            <div className="player-bar opponent-bar w-full max-w-[480px] p-3 rounded-xl bg-white/5 border border-white/10 flex justify-between items-center">
              <div className="player-info flex items-center gap-3">
                <div className="player-avatar w-10 h-10 rounded-full overflow-hidden border border-accent">
                  <img src="/images/Khadeer.jpeg" alt="Khadeer" className="w-full h-full object-cover" />
                </div>
                <div className="player-details flex flex-col">
                  <span className="player-name font-bold text-sm text-white">Khadeer</span>
                  <span className="player-rating text-xs text-accent font-mono">{engineThinking ? '🤔 Thinking...' : 'ELO 3640'}</span>
                </div>
              </div>
              <div className="captured-pieces flex items-center gap-1">
                {renderCapturedPieces(capturedWhite, 'w')}
              </div>
            </div>

            {/* Chess Board */}
            <div className="chess-board-wrapper w-full max-w-[480px] aspect-square rounded-xl overflow-hidden border-2 border-white/20 shadow-2xl">
              <div className="chess-board grid grid-cols-8 grid-rows-8 w-full h-full">
                {ranks.map((rank) => (
                  files.map((file) => {
                    ...
                    return (
                      <div
                        key={square}
                        className={`chess-square relative flex items-center justify-center cursor-pointer select-none
                          ${isLight ? 'bg-[#eeeed2]' : 'bg-[#769656]'} 
                          ${isSelected ? '!bg-[#baca44]' : ''} 
                          ${isLastMoveSquare ? '!bg-[#aaa23a]' : ''}
                          ${isCheck ? '!bg-[#e65151]' : ''}`}
                      ...
                      >
                        ...
                        {file === (boardFlipped ? 'h' : 'a') && (
                          <span className={`coord-rank absolute top-0.5 left-1 text-[10px] font-bold ${isLight ? 'text-[#769656]' : 'text-[#eeeed2]'}`}>{rank}</span>
                        )}
                        {rank === (boardFlipped ? '8' : '1') && (
                          <span className={`coord-file absolute bottom-0.5 right-1 text-[10px] font-bold ${isLight ? 'text-[#769656]' : 'text-[#eeeed2]'}`}>{file}</span>
                        )}
                        ...
                        {isPossibleMove && (
                          <div className={`move-indicator absolute w-4 h-4 rounded-full bg-black/20 ${piece ? 'w-full h-full rounded-none border-4 border-black/20 bg-transparent' : ''}`} />
                        )}
                      </div>
                    );
                  })
            ...
            <div className="player-bar player-bar-bottom w-full max-w-[480px] p-3 rounded-xl bg-white/5 border border-white/10 flex justify-between items-center">
              <div className="player-info flex items-center gap-3">
                <div className="player-avatar w-10 h-10 rounded-full bg-accent/20 border border-accent flex items-center justify-center text-lg">
                  👤
                </div>
                <div className="player-details flex flex-col">
                  <span className="player-name font-bold text-sm text-white">You</span>
                  <span className="player-rating text-xs text-slate-400">{playerColor === 'w' ? 'White' : 'Black'}</span>
                </div>
              </div>
              <div className="captured-pieces flex items-center gap-1">
          ...
          <div className="chess-side-panel right-panel lg:col-span-3 rounded-3xl bg-white/5 border border-white/10 p-4 flex flex-col justify-between h-[500px] lg:h-[600px]">
            {/* Game Status */}
            <div className="game-status text-center p-3 rounded-xl bg-white/5 border border-white/10 font-bold text-accent font-geist">
              <span className={game.isCheck() ? 'text-red-400 animate-pulse' : ''}>{gameStatus}</span>
            </div>

            {/* Move History */}
            <div className="move-history flex-1 my-4 overflow-y-auto space-y-1 p-2 rounded-xl bg-black/20 font-mono text-sm">
              <div className="move-history-header text-xs text-slate-400 uppercase font-semibold pb-2 border-b border-white/10">Moves</div>
              {formatMoveHistory().map((move, index) => (
                <div key={index} className="move-row grid grid-cols-3 text-xs py-1 hover:bg-white/5 rounded px-2">
                  <span className="move-num text-slate-500">{move.moveNum}.</span>
                  <span className="move-white text-white font-medium">{move.white}</span>
                  <span className="move-black text-accent">{move.black}</span>
                </div>
              ))}
            </div>
            ...
            <div className="game-controls flex gap-3">
              <button onClick={resetGame} className="control-btn flex-1 py-3 rounded-xl bg-white/10 border border-white/20 text-xs font-bold uppercase tracking-wider text-white hover:bg-white/20 transition-all" data-cursor="disable">
                New Game
              </button>
              <button onClick={flipBoard} className="control-btn flex-1 py-3 rounded-xl bg-white/10 border border-white/20 text-xs font-bold uppercase tracking-wider text-white hover:bg-white/20 transition-all" data-cursor="disable">
                Flip Board
              </button>
            </div>
          </div>
